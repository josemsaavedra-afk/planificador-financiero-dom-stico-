...
const server = createServer(async (req,res) => {
  ...
  try { const body=await readFile(file);res.setHeader('Content-Type',mime[path.extname(file)]||'application/octet-stream');res.end(body); }
...
try {
  ...
  await expired.close();
  console.log('PASS: 4 browser scenarios: recovery/mobile/checkpoints/history (1201 states), IndexedDB user/household isolation, PWA offline reload and legacy caches, expired recovery link. Real SDK, mocked HTTP only.');
